...
public class AdminPanel extends JFrame {
    ...
    private void subscribeUser() {
        ...
        if (chat == null || user == null) {
            JOptionPane.showMessageDialog(this, "Please select both a chat and a user");
            return;
        }
        // Get IDs for reloading
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            // Reload entities within the session
            Chat managedChat = session.get(Chat.class, chatId);
            User managedUser = session.get(User.class, userId);

            if (managedChat != null && managedUser != null) {
                // Add user to chat within the session
                managedChat.getUsers().add(managedUser);
                session.update(managedChat);

                tx.commit();
                JOptionPane.showMessageDialog(this, "User subscribed.");
            } else {
                tx.rollback();
                JOptionPane.showMessageDialog(this, "Error: Chat or user not found");
            }
        } catch (Exception e) {
    ...
    private void unsubscribeUser() {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            if (managedChat != null && managedUser != null) {
                // Remove user from chat within the session
                managedChat.getUsers().remove(managedUser);
                session.update(managedChat);
